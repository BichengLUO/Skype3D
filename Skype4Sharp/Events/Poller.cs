...
namespace Skype4Sharp.Events
{
    class Poller
    {
        ...
        public void StartPoll()
        {
            ...
            CancellationToken mainToken = mainCts.Token;
            tokenSources.Add(mainCts);
            Task.Factory.StartNew(() => // Main Poll
            {
                while (true)
                {
                    string rawInfo = "";
                    HttpRequestMessage webRequest = parentSkype.mainFactory.createWebRequest_POST(clientGatewayMessengerDomain + "/v1/users/ME/endpoints/SELF/subscriptions/0/poll", new string[][] { new string[] { "RegistrationToken", parentSkype.authTokens.RegistrationToken } }, Encoding.ASCII.GetBytes(""), "application/json");
            ...
            CancellationTokenSource requestsCts = new CancellationTokenSource();
            CancellationToken requestsToken = requestsCts.Token;
            tokenSources.Add(requestsCts);
            Task.Factory.StartNew(() => // Contact Requests
            {
                while (true)
                {
                    HttpRequestMessage webRequest = parentSkype.mainFactory.createWebRequest_GET("https://api.skype.com/users/self/contacts/auth-request", new string[][] { new string[] { "X-Skypetoken", parentSkype.authTokens.SkypeToken } });
                    string rawInfo = "";
                    using (var handler = new HttpClientHandler() { CookieContainer = parentSkype.mainCookies })
                    using (var client = new HttpClient(handler))
                    {
                    ...
                    if (requestsToken.IsCancellationRequested)
                        break;
                }
            }, requestsToken);
        }
